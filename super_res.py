...
class SuperResRBPN(Network):
    def __init__(self,
                 ...
                 best_validation_loss = None,
                 best_psnr = None,
                 best_model_file = 'best_rbpn_sgd.pth',
                 model_weights = None,
                 optim_weights = None
                 ):
        ...
        self.model = rbpn.Net(num_channels=num_channels, base_filter=base_filter,feat=feat, num_stages=3,
                              n_resblock=n_resblock, nFrames=n_frames, scale_factor=upscale_factor)
        # print(self.model.state_dict().keys())
        if model_weights:
            self.model.load_state_dict(model_weights)
    ...
    def batch_to_loss(self,data_batch):
        inputs,target,neighbours,flow = data_batch[0],data_batch[1],data_batch[2],data_batch[3]
        inputs = inputs.to(self.device)
        target = target.to(self.device)
        neighbours = [n.to(self.device).float() for n in neighbours]
        flow = [f.to(self.device).float() for f in flow]
        # print(inputs.shape, neighbours[0].shape, flow[0].shape)
    ...
    def evaluate(self,dataloader, **kwargs):
        ...
        with torch.no_grad():
            for data_batch in dataloader:
                img, hr_target, neighbours, flow, hr_resized = data_batch[0],data_batch[1],data_batch[2],data_batch[3],data_batch[4]
                img = img.to(self.device)
                hr_target = hr_target.to(self.device)
                neighbours = [n.to(self.device).float() for n in neighbours]
                flow = [f.to(self.device).float() for f in flow]
                hr_super_res = self.forward(img, neighbours, flow)
                _,loss_dict = self.compute_loss(hr_super_res,hr_target)
                torchvision.utils.save_image([
                                            #   denorm_tensor(hr_target.cpu()[0], self.img_mean, self.img_std),
                                              hr_target.cpu()[0],
